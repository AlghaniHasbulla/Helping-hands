...
const ApproveRequestsPage = () => {
  ...
  const fetchDonationRequests = useCallback(async () => {
    ...
    try {
      ...
      setDonationRequests(response.data.items || response.data);
    } catch (err) {
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-md font-sans">
      <h1 className="text-3xl font-bold text-gray-800 mb-6">Approve Donation Requests</h1>
      
      {donationRequests.length === 0 ? (
        <p className="text-gray-600 text-center py-8">No approved donation requests to display.</p>
      ) : (
